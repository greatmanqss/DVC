...
void TemporalPoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  sampling_idx_.Reshape(video_num_,15,1,1);// qss
...
void TemporalPoolingLayer<Dtype>::Forward_cpu(
	...
	Dtype* smidx_data = sampling_idx_.mutable_cpu_data();
  ...
  case TemporalPoolingParameter_TemPoolOp_AVG:
    ...
    for(int clip_id = 0; clip_id < video_num_; clip_id++)
    {
		...
		if ((end-start+1) > 15) // qss
		{
			int perm[end-start+1];
			// gen start to end in order
			...
			for (int i=0; i<(end-start+1);i++){
				int j,t;
				j = rand()%(end-start+1-i)+i;
				t = perm[j];
				perm[j] = perm[i];
				perm[i] = t;
			}
			//perm[0] = (end+start)/2;
			// random sampling 15 frames
			for (int idx = 0; idx < 15; idx++){// qss
				int frame_id = start + floor(idx*(end-start+1)/15);// qss
				//int frame_id = perm[idx];
				smidx_data[clip_id*15+idx] = frame_id;// qss
				int offset_bottom = bottom[0]->offset(frame_id);
                caffe_axpy(count, Dtype(1/15.0), bottom[0]->cpu_data()+offset_bottom, top_data+offset_top);// qss
			}
  ...
  case TemporalPoolingParameter_TemPoolOp_MAX:
    ...
    for(int clip_id = 0; clip_id < video_num_; clip_id++)
    {
		...
		if ((end-start+1) > 15)// qss
		{
			...
			int offset_data_b = bottom[0]->offset(start + floor((end-start+1)/15));// qss
			//int offset_data_a = bottom[0]->offset(perm[0]);
			//int offset_data_b = bottom[0]->offset(perm[1]);// qss
			...
			for (int idx = 0; idx < count; ++idx) {
			  if (bottom_data_a[idx] > bottom_data_b[idx]) {
				...
				//mask[idx+offset_top] = perm[0]; 
			  } else {
				top_data[idx+offset_top] = bottom_data_b[idx];  // maxval
				mask[idx+offset_top] = start + floor((end-start+1)/15);  // maxid // qss
				//mask[idx+offset_top] = perm[1];
			  }
			}
			
			
			// random sampling 15 frames
			for (int idx = 2; idx < 15; idx++){// qss
				int frame_id = start + floor(idx*(end-start+1)/15);// qss
				//int frame_id = perm[idx];
				int offset_bottom = bottom[0]->offset(frame_id);
				bottom_data_b = bottom[0]->cpu_data()+offset_bottom;
				for (int idx2 = 0; idx2 < count; ++idx2) {
				  if (bottom_data_b[idx2] > top_data[idx2+offset_top]) {
					top_data[idx2+offset_top] = bottom_data_b[idx2];  // maxval
					mask[idx2+offset_top] = frame_id;  // maxid
				  }
				}
			}
		}
    ...
    if (propagate_down[i]) {
      ...
      case TemporalPoolingParameter_TemPoolOp_AVG:
          for(int clip_id = 0; clip_id < video_num_; clip_id++)
          {
			  ...
			  if ((end-start+1)>15){ // qss
				  for (int idx = 0; idx < 15; idx++){// qss
					  int frame_id = sampling_idx_.cpu_data()[clip_id*15+idx];// qss
				      int offset_bottom = bottom[0]->offset(frame_id);
                      caffe_cpu_scale(count, Dtype(1/15.0), top_diff+offset_top, bottom_diff+offset_bottom);// qss
				  }
